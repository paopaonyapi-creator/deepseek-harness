...
$StatusPath = Join-Path $Root 'dsh-lan-status.json'
...
Assert-True ($enable -match '\$BackendPort\s*=\s*3098') 'enable script must keep the DSH backend on 3098'
Assert-True ($enable -match 'connectport=\$BackendPort') 'enable script must proxy public 3080 to the configured backend port'
...
Assert-True ($enable -match '@local/dsh-apple-mobile') 'enable script must preserve the user-owned Apple mobile UI overlay'
...
Assert-True ($enable -match 'Get-NetTCPConnection.+BackendPort') 'enable script must verify DSH listens on its backend port'
Assert-True ($enable -match 'Invoke-WebRequest.+BackendPort') 'enable script must verify backend HTTP'
Assert-True ($enable -match 'backendUrl') 'status output must identify the internal DSH backend URL'
...
Assert-True (-not $hasUtf8Bom) 'status JSON must be UTF-8 without BOM'
$status = Get-Content -Raw -LiteralPath $StatusPath | ConvertFrom-Json
Assert-True ($status.backendUrl -eq 'http://127.0.0.1:3098/') 'status JSON must identify backend port 3098'
